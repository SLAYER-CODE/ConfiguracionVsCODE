...
function Ventas() {
  ...
  const [rederItem,setRenderItem]=React.useState<Boolean>(false)
  ...
  useEffect(() => {
    ...
    data?.ProductsProfile.map((dato,index)=>{
      ...
      if(dato.products_contrate?.length!=0){
        ...
        console.log(dato.products_contrate)
        console.log([...Locations,...dato.products_contrate!!])
        setLocation([...Locations,...dato.products_contrate!!])
      }
  ...
  return (
    <SafeAreaView
              ...
              return (
                <View>
                  ...
                  <View style={{ width: "100%", padding: 10, backgroundColor: '#FFF', borderWidth: 5, borderColor: 'black', flexDirection: 'row' }}>
                    <Text style={{ flex: 1, textAlignVertical: 'center', justifyContent: 'center' }}>{item.product_name}</Text>
      ...
      <AwesomeButton
        ...
        onPress={(nextt?: AfterPressFn) => { 
          console.log(Locations)
          NavigationHome.navigate("Location",{Locations:Locations})
          nextt?.call(0,(()=>{}))
        }}
     >
