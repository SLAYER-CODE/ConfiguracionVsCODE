...
export class Users extends BaseEntity {
    ...
    @OneToOne(type => Phone, phone_id => phone_id.phone_id, {
        cascade: ["update", "insert","remove"], onUpdate: "CASCADE", onDelete: "CASCADE"
    })
    @Field(() => Phone, { nullable: true })
    @JoinColumn({ name: "user_id" })
    phone_id!: Phone;

    @OneToOne(type => GpsUser, phone_id => phone_id.user_relation, {
        cascade: ["update", "insert","remove"], onUpdate: "CASCADE", onDelete: "CASCADE"
    })
    @Field(() => GpsUser, { nullable: true })
    @JoinColumn({ name: "gps_user",referencedColumnName:"user_relation" })
    gps_user!: GpsUser;
