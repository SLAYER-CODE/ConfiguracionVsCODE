...
class MenuTerminal(object):
    ...
    async def PrintUsers(self,UsersFor):
        ...
        for x in UsersFor:
            ...
            print(InfoNewUsers)
    async def PrintUsersFormated(self,UsersFor):
        # UsersFor = AllUsers[0]["body"].get("results")
        # maxName=len(str(max(UsersFor,key=lambda p:len(p[2]))[2]))
        maxemail=len(str(max(UsersFor, key=lambda p:len(p[0]))[0])) 
        maxpwd=len(str(max(UsersFor, key=lambda p:len(str(p[1])))[1]))
        maxusers=len(str(len(UsersFor)))
        for y,x in enumerate(UsersFor):
            if(len(x)>=3):
                name=('#58FAF4 bold', '{:^{}}'.format(x[2][:21],24) )
            else:
                name=('#58FAF4 bold', '{:^}'.format("Sin Nombre",24))
            InfoNewUsers = FormattedText([
                ('#2EFE64', '|'),
                ('#FE9A2E', '[{:0>{}}]'.format(y,maxusers)),
                ('#2EFE64', '|'),
                name,
                ('#FFFF00', '|'),
                ('#2EFE64 underline', '{:^{}}'.format(x[0], maxemail)),
                ('#2EFE64', '|'),
                ('#A4A4A4', '{:^{}}'.format(str(x[1]),  maxpwd)) ,
                ('#2EFE64', '|'),
            ])
            # self.logger.info("\n\tUsuario: {} {} \n\tCorrero: {} \n\tAvatar: {}".format(user["givenName"],user["familyName"],user["userName"],user["avatar"]["permanentUrl"]))
            print(InfoNewUsers)
            
    async def InsertPwdUser(self, UsersFor, CurseSelectUser):
        # usuarios = []
        # UsersFor = AllUsers[0]["body"].get("results")
        formatUser=[]
        await self.PrintUsers(UsersFor)
        self._completer = ["Virutal22", "Virutal21", "Virutal20", "Virutal18"]
        self.message = ["SLAYER",[self.nodeprev.Session[x].username for x in CurseSelectUser],
        ...
        for x in UsersFor:
            user = x["user"]
            formatUser.append([user["userName"],
                             contraseña,
    ...
    async def SelectCurse(self, curses):
        ...
        for x, y in enumerate(curses, 1):
            InfoCurses = FormattedText([
                ...
                ('#2EFE64', '|'),
                ('#FFFFFF bold', '{:█<{}}'.format(
                    str(y[4]), len(str(max(curses, key=lambda s:len(str(s[4])) )[4] )))),
                ('#2EFE64', '|'),
                ('#FFFF00', '{:^{}}'.format(
        ...
        self.message = ["SLAYER", [x.username for x in self.nodeprev.Session], "selecione su curso"]
        ...
        with patch_stdout():
            ...
            CursesSelect = [curses[x] for x in list( map(lambda x: (int(x.strip()) - 1) if(x.strip().isdigit()) else self._completer.index(x.strip()), insert) )]
