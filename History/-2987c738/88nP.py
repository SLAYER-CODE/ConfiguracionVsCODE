...
class Connect:
    ...
    def __init__(self, gmail: str, password: str, name: str="Sin Name", logger: Logger):
    ...
    async def connection(self):
        ...
        burp0_headers = {
            ...
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/"+str(random.randint(80,84))+".0.4147.89 Safari/537.36",
        ...
        burp0_cookies = {
            "JSESSIONID": self.session.cookie_jar._cookies["senati.blackboard.com"]["JSESSIONID"].value,
            "AWSELB": self.session.cookie_jar._cookies["senati.blackboard.com"]["AWSELB"].value,
            "AWSELBCORS": self.session.cookie_jar._cookies["senati.blackboard.com"]["AWSELBCORS"].value,
            "BbRouter": self.session.cookie_jar._cookies["senati.blackboard.com"]["BbRouter"].value,
        ...
        burp0_headers = {
            ...
            "User-Agent": "Mozilla/5.0 (Windows NT "+str(random.randint(7,11))+".0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/84.0.4147.89 Safari/537.36",
            "Sec-Fetch-Site": "same-origin",
            "Sec-Fetch-Mode": "cors",
            "Sec-Fetch-Dest": "empty",
            "Referer": "https://senati.blackboard.com/",
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "es-ES,es;q=0.9"}
        async with self.session.get(burp0_url, headers=burp0_headers, cookies=burp0_cookies) as resp:
            assert resp.status == 200
            self.logger.info(
                "Obteniendo Primeras cokiees ".format(resp.cookies))
    ...
    async def initsessionPrevius(self):
        ...
        burp0_cookies = {
            ...
            "JSESSIONID": self.session.cookie_jar._cookies["senati.blackboard.com"]["JSESSIONID"].value,
            "BbRouter": self.session.cookie_jar._cookies["senati.blackboard.com"]["BbRouter"].value,
            "_ga": "GA1.2.1267852809.1634301288",
            "_gid": "GA1.2.83089956.1634301288",
            "AWSELB": self.session.cookie_jar._cookies["senati.blackboard.com"]["AWSELB"].value,
            "AWSELBCORS": self.session.cookie_jar._cookies["senati.blackboard.com"]["AWSELBCORS"].value,
        ...
        self.logger.debug(
            self.session.cookie_jar._cookies["senati.blackboard.com"])
        async with self.session.get(burp0_url, headers=burp0_headers, cookies=burp0_cookies) as resp:
            assert resp.status == 200
            ...
            self.logger.debug("SamLRequest:{} And Load {}".format(
                SAMLRequestRedirect, SAMLRequestLoad))

        burp0_url = "https://login.microsoftonline.com" + \
            SAMLRequestLoad["urlPost"]
        burp0_cookies = {
            "fpc": self.session.cookie_jar._cookies["login.microsoftonline.com"]["fpc"].value,
            "x-ms-gateway-slice": "estsfd",
            "stsservicecookie": self.session.cookie_jar._cookies["login.microsoftonline.com"]["stsservicecookie"].value,
        ...
        async with self.session.get(burp0_url, headers=burp0_headers, cookies=burp0_cookies) as resp:
            ...
            self.logger.debug("Token Flow HTTP {}".format(
                tokenJsonLoadInitflowToken))

        burp0_url = "https://login.microsoftonline.com:443/common/GetCredentialType?mkt=es-ES"

        burp0_headers = {
            "Connection": "close",
            "hpgrequestid": tokenJsonLoadInitflowToken["sessionId"],
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/84.0.4147.89 Safari/537.36",
            "client-request-id": "1f3c6b63-e326-456e-8b2e-ee7ef8d8a255",
            "canary": tokenJsonLoadInitflowToken["apiCanary"],
        ...
        burp0_json = {
            ...
            "country": "PE",
            "federationFlags": 0,
            "flowToken": tokenJsonLoadInitflowToken["sFT"],
            "forceotclogin": False,
            ...
            "isSignup": False,
            "originalRequest": tokenJsonLoadInitflowToken["sCtx"],
            "username": self.gmail}
        ...
        async with self.session.post(burp0_url, json=burp0_json, headers=burp0_headers, cookies=self.session.cookie_jar._cookies["login.microsoftonline.com"]) as resp:
            ...
            ResGmailFinal
            self.logger.debug(
                "Respuesta al iniciar el GMAIL {}".format(ResGmailFinal))
        ...
        burp0_json = {
            ...
            "ext": {
                "app": {
                    ...
                    "sesId": "1f3c6b63-e326-456e-8b2e-ee7ef8d8a255",
                    "userId": "p: 2000",
                    "ver": "2.1.12108.11"},
                "cloud": {
                    "role": "SCUS",
                    "roleInstance": "SN3XXXX",
                            "roleVer": "2.1.12108.11"},
                "metadata": {
                    "f": {
                        "viewId": {
                            "t": 6}}},
                "sdk": {
                    "ver": "1DS-Web-JS-2.3.4"}},
            "iKey": "o:69adc3c768bd4dc08c19416121249fcc",
            "name": "IDUX_ESTSClientTelemetryEvent_WebWatson",
            "time": "2021-10-15T18:37:34.136Z",
            "ver": "4.0"}
    ...
    async def initRootSession(self):
        ...
        burp0_cookies = {
            "x-ms-gateway-slice": self.session.cookie_jar._cookies["login.microsoftonline.com"]["x-ms-gateway-slice"].value,
            "stsservicecookie": "estsfd",
            "AADSSO": "NA|NoExtension",
            "esctx": self.session.cookie_jar._cookies["login.microsoftonline.com"]["esctx"].value,
            "brcap": "0",
            "buid": self.session.cookie_jar._cookies["login.microsoftonline.com"]["buid"].value,
            "fpc": self.session.cookie_jar._cookies["login.microsoftonline.com"]["fpc"].value,
            "clrc": "{%2218916%22%3a[%220b9gVZOA%22%2c%22hVrkrE7O%22]}",
            "MSFPC": self.session.cookie_jar._cookies["microsoft.com"]["MC1"].value,
            "wlidperf": "FR=L&ST=1634323333312"}
        ...
        burp0_headers = {
            ...
            "Sec-Fetch-User": "?1",
            "Sec-Fetch-Dest": "document",
            "Referer": self.tokenJsonLoadInitflowToken["urlPost"],
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "es-ES,es;q=0.9"}
        burp0_data = {
            "i13": "0",
            ...
            "psRNGCEntropy": '',
            "psRNGCSLK": '',
            "canary": self.tokenJsonLoadInitflowToken["canary"],
            "ctx": self.tokenJsonLoadInitflowToken["sCtx"],
            "hpgrequestid": self.tokenJsonLoadInitflowToken["sessionId"],
            "flowToken": self.tokenJsonLoadInitflowToken["sFT"],
            "PPSX": '',
            "NewUser": "1",
            "FoundMSAs": '',
            "fspost": "0",
            ...
            "i18": '',
            "i19": "339643"}
        self.logger.debug("Data Send {}".format(burp0_data))
        while True:
            self.logger.critical("Se inicio consulta Principal")
            async with self.session.post(burp0_url, headers=burp0_headers, data=burp0_data, cookies=burp0_cookies) as resp:
                assert resp.status == 200
                self.logger.debug("Al iniciar la conexion Cuenta {} {}".format(
                    self.gmail, resp.cookies))
                inicionsession = await resp.text()
                if("ESTSAUTH" not in resp.cookies.keys()):
                    self.logger.warning(
                        "Se reguistro correctamente ubicando los errores...")
                    if(inicionsession.find("mantener") != -1):
                        self.logger.critical(
                            "Si inicio correctamente pero no reguistro las cokies intentando nuevamante...")
                        self.status=Status.Dudoso
                        continue
                    elif(inicionsession.find("recuerda") != -1):
                        self.logger.critical(
                            "La contraseña es incorrecta!! Retirando Usuario...")
                        self.status=Status.Incorrecto
                        
                    elif(inicionsession.find("bloqueado") != -1):
                        self.logger.critical(
                            "Inicio de session bloqueado púede deberse a que la cuenta no esta reguistrada o no esta activada o que se espero mucho el tiempo de espera")
                        self.status=Status.Bloqueado
                    else:
                        self.logger.critical(
                            "No se ubico el error, Inspeccionado...")
                        self.logger.error(inicionsession)
                        self.status=Status.Desconosido
                    return 

                self.bbrouterindex = dict([x.split(
                    ":") for x in self.session.cookie_jar._cookies["senati.blackboard.com"]["BbRouter"].value.split(",")])
                self.logger.debug(
                    "Imprimiendo el index de brouter en las cokiies {}".format(
                        self.bbrouterindex))
            break
        ...
        burp0_cookies = {
            "JSESSIONID": self.session.cookie_jar._cookies["senati.blackboard.com"]["JSESSIONID"].value,
            "BbClientCalenderTimeZone": "America/Lima",
            "_ga": "GA1.2.1015695736.1634337120",
            "_gid": "GA1.2.810803460.1634337120",
            "COOKIE_CONSENT_ACCEPTED": "true",
            "AWSELB": self.session.cookie_jar._cookies["senati.blackboard.com"]["AWSELB"].value,
            "AWSELBCORS": self.session.cookie_jar._cookies["senati.blackboard.com"]["AWSELBCORS"].value,
            "BbRouter": self.session.cookie_jar._cookies["senati.blackboard.com"]["BbRouter"].value,
            "_gat_gtag_UA_84289107_3": "1",
            "JSESSIONID": self.session.cookie_jar._cookies["senati.blackboard.com"]["JSESSIONID"].value}
        ...
        self.logger.debug("Cokiies para la obtencion de datos {}".format(
            self.session.cookie_jar._cookies["senati.blackboard.com"]))
        async with self.session.get(burp0_url, headers=burp0_headers, cookies=burp0_cookies) as resp:
            assert resp.status == 200
            self.logger.debug(
                "Reguistrando Autenticaciones... {}".format(resp.cookies))
            self.RedirectUltra = resp

        self.logger.info(
            "Iniciando con credenciales... USER {}".format(self.gmail))
        
        self.logger.critical(
            self.session.cookie_jar._cookies["login.microsoftonline.com"])
        ...
        burp0_cookies = {
            ...
            "esctx": self.session.cookie_jar._cookies["login.microsoftonline.com"]["esctx"].value,
            "brcap": "0",
            "wlidperf": "FR=L&ST=1634431840371",
            "ESTSAUTHLIGHT": self.session.cookie_jar._cookies["login.microsoftonline.com"]["ESTSAUTHLIGHT"].value,
            "ESTSSC": self.session.cookie_jar._cookies["login.microsoftonline.com"]["ESTSSC"].value,
            "ESTSAUTHPERSISTENT": self.session.cookie_jar._cookies["login.microsoftonline.com"]["ESTSAUTHPERSISTENT"].value,
            "ESTSAUTH": self.session.cookie_jar._cookies["login.microsoftonline.com"]["ESTSAUTH"].value,
            "ch": self.session.cookie_jar._cookies["login.microsoftonline.com"]["ch"].value,
            "fpc": self.session.cookie_jar._cookies["login.microsoftonline.com"]["fpc"].value,
            "clrc": "{%2218917%22%3a[%22NKuhto9Z%22%2c%22YMcTWH6J%22%2c%22mQE/hwuc%22]}"}
        ...
        async with self.session.get(burp0_url, headers=burp0_headers, cookies=burp0_cookies) as resp:
            self.logger.info(
                "STATUS INIT LOCATION MICROSOFT URL {}".format(resp.status))
            assert resp.status == 200
            self.logger.debug(resp.cookies)
    ...
    async def UserDell(self):
        ...
        burp0_headers = {
            ...
            "Sec-Fetch-Mode": "cors",
            "Sec-Fetch-Dest": "empty",
            "Referer": "https://senati.blackboard.com/ultra/course",
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "es-ES,es;q=0.9",
            "Connection": "close"}
    ...
    async def extractCurses(self):
        self.bbrouterindex = dict([x.split(
            ":") for x in self.session.cookie_jar._cookies["senati.blackboard.com"]["BbRouter"].value.split(",")])
        # params = {"expand":"course.effectiveAvailability,course.permissions,courseRole","includeCount":"true","limit":"10000"}
        burp0_url = "https://senati.blackboard.com:443/learn/api/v1/users/" + self.userID + \
            "/memberships?expand=course.effectiveAvailability,course.permissions,courseRole&includeCount=true&limit=10000"
        burp0_cookies = {
            "JSESSIONID": self.session.cookie_jar._cookies["senati.blackboard.com"]["JSESSIONID"].value,
            "_ga": "GA1.2.1649868462.1634421450",
            "_gid": "GA1.2.1305288680.1634421450",
            "COOKIE_CONSENT_ACCEPTED": "true",
            "BbClientCalenderTimeZone": "America/Lima",
            "JSESSIONID": self.session.cookie_jar._cookies["senati.blackboard.com"]["JSESSIONID"].value,
            "AWSELB": self.session.cookie_jar._cookies["senati.blackboard.com"]["AWSELB"].value,
        ...
        burp0_headers = {
            ...
            "Sec-Fetch-Mode": "cors",
            "Sec-Fetch-Dest": "empty",
            "Referer": "https://senati.blackboard.com/ultra/",
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "es-ES,es;q=0.9",
            "Connection": "close"}
    ...
    async def CurseDell(self, idCurse):
        ...
        burp0_headers = {
            ...
            "Sec-Fetch-Mode": "cors",
            "Sec-Fetch-Dest": "empty",
            "Referer": "https://senati.blackboard.com/ultra/courses/" +
            idCurse +
            "/outline",
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "es-ES,es;q=0.9",
    ...
    async def selectCurseDellUser(self, idCurse):
        burp0_url = "https://senati.blackboard.com:443/learn/api/v1/courses/" + \
            idCurse + "/users/" + self.userID + "?expand=courseRole"
        burp0_headers = {
            "X-Blackboard-Xsrf": self.bbrouterindex["xsrf"],
            "Accept": "application/json, text/plain, */*",
            "Sec-Ch-Ua-Mobile": "?0",
    ...
    async def selectCurseDellRoot(self, idCurse):
        ...
        burp0_headers = {
            ...
            "Sec-Fetch-Mode": "cors",
            "Sec-Fetch-Dest": "empty",
            "Referer": "https://senati.blackboard.com/ultra/courses/" + idCurse + "/outline",
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "es-ES,es;q=0.9",
            "Connection": "close"}
        async with self.session.get(burp0_url, headers=burp0_headers, cookies=self.session.cookie_jar._cookies["senati.blackboard.com"]) as resp:
    ...
    async def GetContentCurse(self, idCurse):
        burp0_url = "https://senati.blackboard.com:443/learn/api/v1/courses/" + idCurse + \
            "/contents/ROOT/children?@view=Summary&expand=assignedGroups,selfEnrollmentGroups.group,gradebookCategory&limit=1000"
        burp0_headers = {
            "X-Blackboard-Xsrf": self.bbrouterindex["xsrf"],
            "Accept": "application/json, text/plain, */*",
            "Sec-Ch-Ua-Mobile": "?0",
    ...
    async def extractUserCurse(self, idCurse):
        ...
        burp0_headers = {
            ...
            "Sec-Ch-Ua-Platform": "\"Windows\"",
            "Content-Type": "application/json;charset=UTF-8",
            "Origin": "https://senati.blackboard.com",
            "Sec-Fetch-Site": "same-origin",
            "Sec-Fetch-Mode": "cors",
            "Sec-Fetch-Dest": "empty",
            "Referer": "https://senati.blackboard.com/ultra/course",
            ...
            "Accept-Language": "es-ES,es;q=0.9",
            "Connection": "close"}
        burp0_json = [{"method": "GET", "relativeUrl": "v1/courses/" + idCurse +
                       "/memberships?roleBucket=TAKING?expand=user&limit=10000?expand?'"}]
        ...
        self.logger.info("Se Finalizo la extraccion {}".format(str(UserCurse)[:100]))
        return UserCurse
    ...
    async def extractTeachersCurse(self, idCurse):
        ...
        burp0_headers = {
            ...
            "Connection": "close"}
        burp0_json = [{"method": "GET", "relativeUrl": "v1/courses/" + idCurse + \
            "/memberships?roleBucket=TEACHING?expand=user&limit=10000&isExcludedFromCourseUserActivity=False&includeCount=true"}]
    ...
    async def extractUsersAllCurses(self):
        ...
        burp0_headers = {
            ...
            "Accept-Language": "es-ES,es;q=0.9",
            "Connection": "close"}
        burp0_json = [{"method": "GET", "relativeUrl": "v1/courses/" + x["course"]["id"] + \
            "/memberships?roleBucket=TAKING?expand=user&limit=10000&isExcludedFromCourseUserActivity=False&includeCount=true"} for x in self.AllCursesUser["results"]]
    ...
    async def extractTeachersAllCurses(self):
        ...
        burp0_headers = {
            ...
            "Connection": "close"}
        burp0_json = [{"method": "GET", "relativeUrl": "v1/courses/" + x["course"]["id"] + \
            "/memberships?roleBucket=TEACHING?expand=user&limit=10000&isExcludedFromCourseUserActivity=False&includeCount=true"} for x in self.AllCursesUser["results"]]
    ...
    async def selectItemCurseAction(self, curse, item):
        burp0_url = "https://senati.blackboard.com:443/learn/api/v1/courses/" + curse + "/contents/" + item + \
            "/children?@view=Summary&expand=assignedGroups,selfEnrollmentGroups.group,gradebookCategory&limit=10"
        burp0_headers = {
            "X-Blackboard-Xsrf": self.bbrouterindex["xsrf"],
            "Accept": "application/json, text/plain, */*",
            "Sec-Ch-Ua-Mobile": "?0",
            ...
            "Sec-Fetch-Mode": "cors",
            "Sec-Fetch-Dest": "empty",
            "Referer": "https://senati.blackboard.com/ultra/courses/" + curse + "/outline",
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "es-ES,es;q=0.9",
            "Connection": "close"}
        async with self.session.get(burp0_url, headers=burp0_headers, cookies=self.session.cookie_jar._cookies["senati.blackboard.com"])as resp:
    ...
    async def selectItemCurseActionItem(self, curse, seconditem):
        burp0_url = "https://senati.blackboard.com:443/learn/api/v1/courses/" + curse + "/contents/" + \
            seconditem + "?expand=assignedGroups,selfEnrollmentGroups.group,alignedGoals,gradebookCategory"
        burp0_headers = {
            "X-Blackboard-Xsrf": self.bbrouterindex["xsrf"],
            "Accept": "application/json, text/plain, */*",
            "Sec-Ch-Ua-Mobile": "?0",
    ...
    async def getExamenGeneral(self, curse, column):
        ...
        burp0_url = "https://senati.blackboard.com:443/learn/api/v1/courses/" + curse + \
            "/gradebook/columns/" + column + "/grades?expand=attemptsLeft&userId=" + self.userID
    ...
    async def getExamenIntentsStatus(self, curse, columns, grades):
        burp0_url = "https://senati.blackboard.com:443/learn/api/v1/courses/" + curse + \
            "/gradebook/columns/" + columns + "/grades/" + grades + \
            "/attempts?fields=id,status,attemptDate"
        burp0_headers = {
            "X-Blackboard-Xsrf": self.bbrouterindex["xsrf"],
            "Accept": "application/json, text/plain, */*",
            "Sec-Ch-Ua-Mobile": "?0",
            ...
            "Referer": "https://senati.blackboard.com/ultra/courses/" +
            curse +
            "/outline/assessment/_9672235_1/overview/attempt/_21982189_1?courseId=" +
            curse,
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "es-ES,es;q=0.9",
            "Connection": "close"}
    ...
    async def getAllIntentsExamDell(self, curse, columns, grades):
        ...
        burp0_url = "https://senati.blackboard.com:443/learn/api/v1/courses/" + curse + \
            "/gradebook/columns/" + columns + "/grades/" + \
            grades + "/attempts?expand=toolAttemptDetail"
        ...
        burp0_headers = {
            ...
            "Referer": "https://senati.blackboard.com/ultra/courses/" +
            curse +
            "/outline/assessment/_9672235_1/overview?courseId=" +
            curse,
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "es-ES,es;q=0.9",
            "Connection": "close"}
