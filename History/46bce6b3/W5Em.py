...
class FileTypeAsync(object):
    ...
    def __call__(self, string):
        try:
            return aiofiles.open(string,mode= self._mode,buffering= self._bufsize,encoding= self._encoding,
                        errors = self._errors)
        except OSError as e:
...
if __name__ == '__main__':
    ...
    haking_bruteforcer.add_argument(
        "--limitUserProcess",
        "-lup",
        help="Inserte el nodo que quieres que tenga cada proceso, 15 procesos pro defecto ejecutando a la vez default [15]",
    ...
    haking_bruteforcer.add_argument(
        ...
        dest='filehakingsave',
        metavar='users.txt',
        help='Archivo que guardara los datos hakeados en formato de entrada',
        default='users.txt')
    haking_bruteforcer.add_argument(
        '--filesaveAnalitict',
        '-fsal',
        dest='filesaveanaliticextractuser',
        metavar='users.txt',
        help='Archivo que guardara los datos hakeados en formato de entrada',
        default=None)
    ...
    haking_bruteforcer.add_argument(
        '--filexcludeusers',
        '-feu',
        dest='fileexludehaking',
        metavar='exlude.txt',
        help='Archivo que se exluira a la hora de hakear todas las cuentas',
        default=None)
    ...
    haking_bruteforcer.add_argument(
        ...
        dest='asyncPasswords',
        action="store_true",
        help='Si decea ejecutar los passwords en cada usuario en cada hilo diferente XD [BURTAL]')
    
    haking_bruteforcer.add_argument(
        '--limitproccespassworduser',
        '-lpwu',
        dest='asynclimitproccespassword',
        type=int,
        metavar='1',
        help='Cuanto deberia de ser el procesamiento de cada password [1,2,3,4] << enbase a tubos como el gusanito por defecto [3]',
        default=3)

    haking_bruteforcer.add_argument(
        '--pwdchangeTimeuser','-pcTu',
        dest='asyncTimepassworduser',
        action="store",
        type=float,
        metavar='0.2',
        help='Cada cuanto tendra un tiempo para iniciar la prueva de la otra contraseña default [0.0]',
        default=0.0)
                ...
                if(args.metoduser in ["!unique", "!U"]):
                    ...
                    if(args.password is None or args.email is None):
                        ...
                        AsincroneSession = Asincrone(
                            args,idcurses=args.curse, idsubitem=args.subCurse, logger=logger)
                    ...
                    if(args.usuariosFile):
                        ...
                        for y, x in enumerate(args.usuariosFile.readlines()):
                            if(x[0] == ":"):
                                ...
                                if(x[1:].rstrip("\n") == "users"):
                                    ...
                                    if(args.usersanaliticacces):
                                        break
                                    
                            else:
                                if(command == 1):
                                    if(x[0] != "#" and x[0] != "~"):
                                        datos=list(map(lambda a: a.rstrip(), x.rstrip("\n").split("|")[:3]))
                        ...
                        loop.run_until_complete(
                            AsincroneSession.mainUsersMany(users))
                    else:
