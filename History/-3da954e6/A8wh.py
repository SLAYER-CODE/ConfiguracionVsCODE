    ...
    async def OneProcessHaking(self, user: Connect):
        ...
        if(self.TArgs.asyncPasswords):
            ...
            for y,x in enumerate(user[1]):
                ...
                newusers[y%self.TArgs.asynclimitproccespassword].append(Connect(gmail=user[0],password=x,logger=self.logger))
            #newusers=[[1,3],[2,4]]
            ...
            for y,pwd in enumerate(user[1]):
                newusers.append(Connect(gmail= user[0],password= pwd , logger= self.logger))
