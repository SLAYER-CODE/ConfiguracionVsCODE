...
class Asincrone(object):
            ...
            if(SelectCurse == []):
                ...
                if(CurseAction == 0):
                    ...
                    if(self.TArgs.passwordHaking is None and self.TArgs.archivePwd is None):
                        NewUsersFormat = await self.MenuTerminal.InsertPwdUser(AllUsersForCurser, SelectCurse[0][4])
                    else:
                        NewUsersFormat = []
                        passwords=[]
                    ...
                    with open("UsersFormateFinal.XD","a") as papu:
    ...
    async def mainHakingUserExtraBruteForce(self,users):
        ...
        if(users!=[]):
            ...
            async with self.TArgs.filehakingsave as ficher:
    ...
    async def SaveUserOpen(self, users: Connect, CursesUser,userLast):
        self.loggerApp.info("Guardando Datos... en {}".format(self.TArgs.filehakingsave))
        
        async with aiofiles.open(self.TArgs.filehakingsave,mode="a") as ficher:
            stringer = "\n"
            if(users!=None):
