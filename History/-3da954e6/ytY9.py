...
class Asincrone(object):
    ...
    async def InitiNewUsers(self, AllUsersCurse):
        if(self.TArgs.passwordHaking==None and self.TArgs.archivePwd==None):
            ...
            await self.mainUsersMany(AllUsersCurse)
        else:
    ...
    async def ExtractSubCurses(self, CurseID):
        ...
        # SubItemCurse =await asyncio.gather(*[ self.getSubItemCurse([x], CurseID) for x in self.Session ])
        # resultados=[]
        # for x in SubItemCurse:
        #     if(x!=None):
        #         resultados.append(x)
        SubItemCurse =await self.getSubItemCurse(self.Session, CurseID)
        SubItemItemCurse = await self.getSubItemItemCurse([self.Session[0]], CurseID, SubItemCurse[0])
