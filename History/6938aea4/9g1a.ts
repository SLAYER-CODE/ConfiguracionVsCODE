...
// };
  ...
  async ProductsProfileContrate(@Ctx() ctx: ContextAuth) {
    ...
    const dat = await Products.find({where:{user_relation:{uid:ctx.user.id},products_contrate:null},relations:["user_relation","gps_relation"],order:{update_product:"DESC"}});
    console.log("Obtener")
    return dat;
  }
}
