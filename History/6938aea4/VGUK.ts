...
// };
  ...
  async ProductsProfile(@Ctx() ctx: ContextAuth) {
    ...
    // Products.getRepository().find
    // const dat = await Products.find({where:{user_relation:{uid: ctx.user.id}}});
    const dat = await Products.find({where:{user_relation:{uid:ctx.user.id}},relations:["user_relation"],order:{update_product:"DESC"}});
    console.log("Obtener")
    return dat;
  }
...
export class LocationServiceResolver {
  @Authorized(["USER", "UNDEFINED"])
  @Query(() => [GpsServices])
  Categories(@Ctx() ctx: ContextAuth): Promise<GpsServices[]> {
    const Categorys = GpsServices.find();
    loggerResolvers.info("Peticion del usuario: " + ctx.user);
    loggerResolvers.info("Retornando todas las categorias: " + Categorys);
    return Categorys;
  }
