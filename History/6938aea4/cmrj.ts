...
// };
  ...
  async ProductsProfileContrate(@Ctx() ctx: ContextAuth) {
    ...
    const dat = await Products.find({relations:["user_relation","gps_relation"],order:{update_product:"DESC"},join:{alias:"user",leftJoinAndSelect:{"user.user_id":"products_contrate"}} });
    dat.map((a,index)=>{
      console.log(a)
    })
    return dat;
