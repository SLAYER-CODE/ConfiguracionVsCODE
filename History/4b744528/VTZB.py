...
def serializexd(analicuser:str,search):
    sinfoni=[]
    with open(analicuser,"r") as archive:
        for x in archive.read().splitlines():
...
def main():
    ...
    # analiticArchive("UsersExtractSubUsers.txt","UserFilterImportantCloudComputing.txt")
    
    # analiticArchive("UsersExtractCloudComputingFinally.txt","UsersFilterImportant.txt")

    #SIRVE PRA INSPECCIONAR LOS CURSOS EN UN ARCHIVO BUSCANDO Y SERIALIZANDO O DESEREALIZANDO
    serializado=serializeUsers("UsersExtractFilerFinally.txt")
    print("FInal")
    final=SearchCurseInUsers(serializado,"Negocios")
    finalfinal=serializexd("UsersExtractFilerFinally.txt","Dataware")
    print("Final 2")
    deserializeusers(final,"UsersFilterDateImportant.txt")
    print(final)
