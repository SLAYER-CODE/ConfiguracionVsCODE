...
import { GET_LOCATION_SERVICES } from '../../../GraphQl/Queries';
import { useQuery } from '@apollo/client';
import { Get_Location_Services, Get_Location_Services_GpsServices } from '../../../GraphQl/Types';
function MessageTab() {
    const { loading, error, data, refetch } = useQuery<Get_Location_Services>(GET_LOCATION_SERVICES);
    const [locations, setLocations] = React.useState<Get_Location_Services_GpsServices[]>();
    React.useEffect(() => {
        if (data != undefined) {
            console.log(data);
            setLocations(data.GpsServices);
        }
    }, [data]);
            ...
            <View style={styles.container}>
                ...
                <MapboxGL.MapView onPress={(b) => { console.log((b.geometry as any).coordinates) }} localizeLabels={true} logoEnabled={false} style={styler.map} >
                    ...
                    {   
                        locations != undefined ? (
                            asd=generateColor(),
                            locations.map((e: Get_Location_Services_GpsServices, index) => {
                                 { console.log("##################3333##################3333##################3333##################3333##################3333##################3333##################3333") }
                                        console.log(parseFloat(e.latitud))
                                        console.log(e.latitud)
                                        console.log(parseFloat(e.longitud))
                                        console.log(e.longitud)
                                return (
                                    <MapboxGL.MarkerView onSelected={() => {
                                       
                                    }} id={index.toString()} coordinate={[parseFloat(e.latitud), parseFloat(e.longitud)]}>
                                        <View>
                                            <View style={[stylera.markerContainer,{backgroundColor:asd,borderRadius:50,justifyContent:'center',padding:10}]}>
                                                <View >
                                                    <Text style={{textShadowColor:'black',color:'white',textShadowRadius:5,fontWeight:'bold',fontSize:6,textAlign:'center'}}> {e.gps_relation?.product_name}</Text>
                                                </View>
                                            </View>
                                        </View>
                                    </MapboxGL.MarkerView>
