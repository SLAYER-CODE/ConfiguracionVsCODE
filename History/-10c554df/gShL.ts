...
export class Products extends BaseEntity {
    ...
    @ManyToMany(type => Brands, brands_products => brands_products.brand_id, {
        cascade: ["remove", "update","insert",], onUpdate: "CASCADE", onDelete: "CASCADE",
    })
    @Field(() => [Brands])
    ...
    // @Column({name:"user_id"})
    @ManyToOne(type => Users, user_id => user_id.user_id, {
        cascade: ["remove", "update"], onUpdate: "CASCADE", onDelete: "CASCADE", createForeignKeyConstraints: true
    })
    @Field(() => Users)
    @JoinColumn({ name: "user_relation", referencedColumnName: "id"})
    // @TypeormLoader()
